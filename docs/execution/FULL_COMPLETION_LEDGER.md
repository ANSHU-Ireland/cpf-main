...
| CPF-06 | Replace web process-local stores with authenticated API calls | In progress | RUN-02, REV-08 and employer campaign lifecycle use scoped sessions and PostgreSQL; remaining routes are pending                  |
| CPF-07 | Wire 244 OpenAPI operations to concrete handlers              | In progress | runtime, scorecard and campaign CRUD/lifecycle operations are concrete; the remaining compatibility routes are pending           |
| CPF-08 | PostgreSQL repositories and transaction/outbox coverage       | In progress | runtime, scorecard and campaign writes have tenant-scoped audit/outbox evidence; remaining repositories are pending              |
...
| CPF-13 | Deterministic PostgreSQL demo seed                            | In progress | seed idempotently creates runtime/review data, three campaigns and three scoped sessions; remaining journeys are pending         |
...
The canonical RUN-02 candidate flow, REV-08 human criterion scorecard and employer campaign list/detail/lifecycle now read and write through the explicit demo API into PostgreSQL. The repositories apply tenant context, write audit records and enqueue transactional outbox events atomically. The server resolves hashed, expiring candidate/reviewer/admin sessions from IAM tables and checks the resource scope before domain authorization. Live probes prove missing credentials return 401, assigned resources return 200, and cross-role access returns 403. Browser verification proves responses, flags, rubric scores, rationales and campaign transitions rehydrate from PostgreSQL with `ai_observation_id` remaining null. The Northstar seed remains idempotent at two attempts, six responses, one reviewer assignment, three criterion scores, three campaigns and three active scoped sessions.
...
1. The generic server dispatcher now has concrete runtime, scorecard and campaign CRUD/lifecycle operations, but the remaining OpenAPI operations still return compatibility responses instead of invoking every `apps/api` handler.
2. RUN-02, REV-08 and employer campaigns are persistent and session-protected; the remaining web routes and read models still use process-local synthetic stores.
