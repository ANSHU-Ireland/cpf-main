...
| CPF-07 | Wire 244 OpenAPI operations to concrete handlers              | In progress | runtime, scorecard, campaign, candidate-detail and invitation-command operations are concrete; compatibility routes remain      |
| CPF-08 | PostgreSQL repositories and transaction/outbox coverage       | In progress | runtime, scorecard, campaign and invitation writes have tenant-scoped audit/outbox evidence; remaining repositories are pending |
...
1. The generic server dispatcher now has concrete runtime, scorecard, campaign, candidate-detail and invitation-command operations, but the remaining OpenAPI operations still return compatibility responses instead of invoking every `apps/api` handler.
